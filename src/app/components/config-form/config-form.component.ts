      ...
      <div class="header">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            [attr.aria-label]="'HEADER.RESET_LABEL' | translate"
            [attr.title]="'HEADER.RESET_TITLE' | translate"
          >
          ...
          <button
            mat-icon-button
            (click)="shareConfig()"
            [attr.aria-label]="'HEADER.SHARE_LABEL' | translate"
            [attr.title]="'HEADER.SHARE_TITLE' | translate"
          >
      ...
      <form [formGroup]="configForm" class="config-form">
        <mat-tab-group animationDuration="0ms" class="flex-1">
          ...
          <mat-tab [label]="'TABS.GENERAL' | translate">
            <div class="tab-content">
              ...
              <div class="flex flex-col gap-4 mt-4">
                <mat-slide-toggle formControlName="treks" color="primary">{{
                  'FIELDS.TREKS' | translate
                }}</mat-slide-toggle>
                <mat-slide-toggle formControlName="outdoor" color="primary">{{
                  'FIELDS.OUTDOOR' | translate
                }}</mat-slide-toggle>
                <mat-slide-toggle formControlName="touristicContents" color="primary">{{
                  'FIELDS.TOURISTIC_CONTENTS' | translate
                }}</mat-slide-toggle>
                <mat-slide-toggle formControlName="touristicEvents" color="primary">{{
                  'FIELDS.TOURISTIC_EVENTS' | translate
                }}</mat-slide-toggle>
              </div>
            </div>
          </mat-tab>
            ...
            <div class="tab-content">
              ...
              <mat-form-field appearance="fill" class="w-full">
                ...
                <mat-select
                  ...
                  multiple
                  [disabled]="districtsState().loading"
                >
              ...
              <mat-form-field appearance="fill" class="w-full">
                ...
                <mat-select
                  ...
                  multiple
                  [disabled]="practicesState().loading"
                >
                  @for (item of practicesState().data; track item.id) {
                    <mat-option [value]="item.id">{{
                      item.name[currentLanguage() ?? 'fr']
                    }}</mat-option>
                  }
                </mat-select>
              ...
              <mat-form-field appearance="fill" class="w-full">
                ...
                <mat-select
                  ...
                  multiple
                  [disabled]="structuresState().loading"
                >
          ...
          <mat-tab [label]="'TABS.APPEARANCE' | translate" [disabled]="!isApiValid()">
            <div class="tab-content">
              <div class="mb-4">
                <mat-slide-toggle formControlName="rounded" color="primary">{{
                  'FIELDS.ROUNDED' | translate
                }}</mat-slide-toggle>
              </div>
              <label class="block mb-2 text-sm font-medium text-gray-700">{{
                'COLORS.THEME_TITLE' | translate
              }}</label>
                ...
                <mat-select [formControl]="themeControl">
                  @for (theme of themes; track theme.value) {
                    <mat-option [value]="theme.value">{{
                      'COLORS.THEME_LABELS.' + theme.value.toUpperCase() | translate
                    }}</mat-option>
                  }
                </mat-select>
              </mat-form-field>
              ...
              <div class="mb-4">
                <button
                  mat-stroked-button
                  (click)="showAdvancedColors.set(!showAdvancedColors())"
                  type="button"
                >
                  <mat-icon>{{ showAdvancedColors() ? 'expand_less' : 'expand_more' }}</mat-icon>
                  {{
                    (showAdvancedColors() ? 'COLORS.HIDE_ADVANCED' : 'COLORS.SHOW_ADVANCED')
                      | translate
                  }}
                </button>
              </div>
              ...
              @if (showAdvancedColors()) {
                <label class="block mb-2 text-sm font-medium text-gray-700">{{
                  'COLORS.COLORS_TITLE' | translate
                }}</label>
                <div class="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.PRIMARY_APP' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorPrimaryApp"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorPrimaryApp')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorPrimary')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.ON_PRIMARY' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorOnPrimary"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorOnPrimary')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.PRIMARY_CONTAINER' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorPrimaryContainer"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorPrimaryContainer')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.ON_PRIMARY_CONTAINER' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorOnPrimaryContainer"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorOnPrimaryContainer')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.SECONDARY_CONTAINER' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorSecondaryContainer"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorSecondaryContainer')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.ON_SECONDARY_CONTAINER' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorOnSecondaryContainer"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorOnSecondaryContainer')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.BACKGROUND' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                      ...
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorBackground')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorSurface"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorSurface')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.ON_SURFACE' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorOnSurface"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorOnSurface')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.SURFACE_VARIANT' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorSurfaceVariant"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorSurfaceVariant')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.ON_SURFACE_VARIANT' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorOnSurfaceVariant"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorOnSurfaceVariant')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.SURFACE_CONTAINER_HIGH' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorSurfaceContainerHigh"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorSurfaceContainerHigh')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.SURFACE_CONTAINER_LOW' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorSurfaceContainerLow"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorSurfaceContainerLow')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.FAB_BACKGROUND' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="fabBackgroundColor"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('fabBackgroundColor')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.FAB_COLOR' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="fabColor"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                ...
                <label class="block mb-2 text-sm font-medium text-gray-700 mt-4">{{
                  'COLORS.MAP_COLORS_TITLE' | translate
                }}</label>
                <div class="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.TREK_LINE' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorTrekLine"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorTrekLine')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.SENSITIVE_AREA' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorSensitiveArea"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorSensitiveArea')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    <label class="text-xs text-gray-500">{{
                      'COLORS.OUTDOOR_AREA' | translate
                    }}</label>
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorOutdoorArea"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorOutdoorArea')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorMarkers"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorMarkers')?.value
                      }}</span>
                    </div>
                  </div>
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      <input
                        type="color"
                        formControlName="colorClusters"
                        class="h-8 w-14 cursor-pointer rounded border border-gray-300 p-1"
                      />
                      <span class="font-mono text-xs">{{
                        configForm.get('colorClusters')?.value
                      }}</span>
                    </div>
                  </div>
                </div>
          ...
          <mat-tab [label]="'TABS.MAP' | translate" [disabled]="!isApiValid()">
            <div class="tab-content">
              ...
              <mat-form-field appearance="fill" class="w-full">
                ...
                <mat-icon
                  matSuffix
                  [matTooltip]="'FIELDS.LAYER_NAME_TOOLTIP' | translate"
              ...
              <mat-form-field appearance="fill" class="w-full">
                ...
                <mat-icon
                  matSuffix
                  [matTooltip]="'FIELDS.LAYER_URL_TOOLTIP' | translate"
                  matTooltipClass="multiline-tooltip"
                  class="help-icon"
                  >help_outline</mat-icon
                >
              ...
              <mat-form-field appearance="fill" class="w-full">
                ...
                <mat-icon
                  matSuffix
                  [matTooltip]="'FIELDS.ATTRIBUTION_TOOLTIP' | translate"
                  matTooltipClass="multiline-tooltip"
                  class="help-icon"
                  >help_outline</mat-icon
                >
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <mat-form-field appearance="fill" class="w-full">
                  ...
                  <mat-icon
                    matSuffix
                    [matTooltip]="'FIELDS.COMMON_MARKER_TOOLTIP' | translate"
                    matTooltipClass="multiline-tooltip"
                    class="help-icon"
                    >help_outline</mat-icon
                  >
...
export class ConfigFormComponent {
  ...
  configForm = new FormGroup({
    ...
    // Appearance
    colorPrimaryApp: new FormControl(DEFAULT_CONFIG.colorPrimaryApp || '#6750a4', {
      nonNullable: true,
    }),
    colorPrimary: new FormControl(DEFAULT_CONFIG.colorPrimary || '#6750a4', { nonNullable: true }),
    colorOnPrimary: new FormControl(DEFAULT_CONFIG.colorOnPrimary || '#ffffff', {
      nonNullable: true,
    }),
    colorSurface: new FormControl(DEFAULT_CONFIG.colorSurface || '#1c1b1f', { nonNullable: true }),
    colorOnSurface: new FormControl(DEFAULT_CONFIG.colorOnSurface || '#49454e', {
      nonNullable: true,
    }),
    colorSurfaceVariant: new FormControl(DEFAULT_CONFIG.colorSurfaceVariant || '#fef7ff', {
      nonNullable: true,
    }),
    colorOnSurfaceVariant: new FormControl(DEFAULT_CONFIG.colorOnSurfaceVariant || '#1c1b1f', {
      nonNullable: true,
    }),
    colorPrimaryContainer: new FormControl(DEFAULT_CONFIG.colorPrimaryContainer || '#eaddff', {
      nonNullable: true,
    }), // Updated default from model
    colorOnPrimaryContainer: new FormControl(DEFAULT_CONFIG.colorOnPrimaryContainer || '#21005e', {
      nonNullable: true,
    }),
    colorSecondaryContainer: new FormControl(DEFAULT_CONFIG.colorSecondaryContainer || '#e8def8', {
      nonNullable: true,
    }),
    colorOnSecondaryContainer: new FormControl(
      DEFAULT_CONFIG.colorOnSecondaryContainer || '#1d192b',
      { nonNullable: true },
    ),
    colorBackground: new FormControl(DEFAULT_CONFIG.colorBackground || '#fef7ff', {
      nonNullable: true,
    }),
    colorSurfaceContainerHigh: new FormControl(
      DEFAULT_CONFIG.colorSurfaceContainerHigh || '#ece6f0',
      { nonNullable: true },
    ),
    colorSurfaceContainerLow: new FormControl(
      DEFAULT_CONFIG.colorSurfaceContainerLow || '#f7f2fa',
      { nonNullable: true },
    ),
    fabBackgroundColor: new FormControl(DEFAULT_CONFIG.fabBackgroundColor || '#eaddff', {
      nonNullable: true,
    }),
    ...
    touristicContents: new FormControl(DEFAULT_CONFIG.touristicContents ?? false, {
      nonNullable: true,
    }),
    touristicEvents: new FormControl(DEFAULT_CONFIG.touristicEvents ?? false, {
      nonNullable: true,
    }),
    outdoor: new FormControl(DEFAULT_CONFIG.outdoor ?? false, { nonNullable: true }),

    // Map
    colorTrekLine: new FormControl(DEFAULT_CONFIG.colorTrekLine || '#6b0030', {
      nonNullable: true,
    }),
    colorSensitiveArea: new FormControl(DEFAULT_CONFIG.colorSensitiveArea || '#4974a5', {
      nonNullable: true,
    }),
    colorOutdoorArea: new FormControl(DEFAULT_CONFIG.colorOutdoorArea || '#ffb700', {
      nonNullable: true,
    }),
    colorMarkers: new FormControl(DEFAULT_CONFIG.colorMarkers || '#6750a4', { nonNullable: true }),
    colorClusters: new FormControl(DEFAULT_CONFIG.colorClusters || '#6750a4', {
      nonNullable: true,
    }),

    mainMarkerSize: new FormControl(DEFAULT_CONFIG.mainMarkerSize || 32, { nonNullable: true }),
    selectedMainMarkerSize: new FormControl(DEFAULT_CONFIG.selectedMainMarkerSize || 48, {
      nonNullable: true,
    }),
    ...
    departureArrivalMarkerSize: new FormControl(DEFAULT_CONFIG.departureArrivalMarkerSize || 14, {
      nonNullable: true,
    }),
    pointReferenceMarkerSize: new FormControl(DEFAULT_CONFIG.pointReferenceMarkerSize || 24, {
      nonNullable: true,
    }),
  });
  ...
  private createResourceSignal(method: (url: string) => Observable<any>) {
    return toSignal(
      this.configForm.controls.api.valueChanges.pipe(
        ...
        switchMap((url) =>
          method.call(this.geotrekService, url).pipe(
      ...
      map(([formValue, params]: [any, any]) => {
        ...
        const isFormDefault = Object.keys(formValue).every((key) => {
          ...
          if (Array.isArray(currentValue)) {
            ...
            return (
              currentValue.length === defaultArray.length &&
        ...
        const hasConfigParams = configKeys.some((key) =>
  ...
  constructor() {
    ...
    this.configForm.controls.api.valueChanges.subscribe((val) => {
      if (this.isPatching) return;
      // We must clear districts explicitly.
    ...
    colorControls.forEach((controlName) => {
      this.configForm.get(controlName)?.valueChanges.subscribe(() => {
        if (!this.isApplyingTheme && !this.isPatching && this.themeControl.value !== 'custom') {
    ...
    effect(() => {
      ...
      if (config) {
        ...
        this.configForm.patchValue(
          ...
          { emitEvent: true },
        ); // Ensure valueChanges triggers for API to load lists
        ...
        this.isPatching = false; // Ensure flag is reset
      }
    });
  }
  ...
  shareConfig() {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard
        ...
        .then(() => {
          this.snackBar.open(
            this.translationService.translate('MESSAGES.CONFIG_COPIED'),
            undefined,
            {
              duration: 2000,
            },
